...
def uniformCostSearch(problem):
    ...
    "*** YOUR CODE HERE ***"
    return HeuristicSearches(problem,None)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    "*** YOUR CODE HERE ***"
    return HeuristicSearches(problem,heuristic)
...
def HeuristicSearches(problem,heuristic):
    ...
    while(not problem.isGoalState(curr_state)):
        if(curr_state not in closed):
            closed.append(curr_state)
            for Successor in problem.getSuccessors(curr_state):
                cost=problem.getCostOfActions(finalPath+[Successor[1]])

                if heuristic is not None:
                    cost += heuristic(Successor[0], problem)
                if Successor[0] not in closed:
                    open.push(Successor[0],cost)
                    currentPath.push(finalPath + [Successor[1]], cost)
